...
export async function installFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    fileReader.onload = async(e: ProgressEvent<FileReader>): Promise<boolean | void | Error> => {
      // Write archive to temp directory
      if(!fs.existsSync(tmpFile)) {
        fs.writeFileSync(tmpFile, new Uint8Array(Buffer.from(e.target?.result as ArrayBuffer)));
      }
      ...
      if (sevenExtensions.findIndex((ext: string) => ext === extension) !== -1) {
        ...
        fs.writeFileSync(output, new Uint8Array(Buffer.from(e.target?.result as ArrayBuffer)));
        store.commit('setLoadingStatus', true);
        fs.rmdirSync(tmpPath, { recursive: true });
        resolve(true);
      }
