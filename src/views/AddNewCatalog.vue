<template>
  <v-container>
    <v-row>
      ...
      <v-col cols="12">
        <v-card>
          <v-form ref="form" @submit.prevent="submitNewCatalog">
            <v-list-item>
              <v-list-item-content>
                <v-text-field
                  :value="title"
                  :rules="[
                    (value) => !!value || this.$t('addCatalog.formTitleHint'),
                  ]"
              ...
              <v-list-item-avatar size="80" :color="color">
            ...
            <v-card-text>
              ...
              <v-file-input
                ...
                :rules="[
                  (value) => !!value.path || this.$t('addCatalog.formFolder'),
                ]"
                solo
                webkitdirectory
                @change="
                  (file) => {
                    path = file != undefined ? file.path : ''
                  }
                "
              >
            ...
            <v-card-actions>
              ...
              <v-btn color="primary" type="submit" :loading="inProgress">
                {{ $t('app.buttons.add') }}
              </v-btn>
            </v-card-actions>
          </v-form>
        </v-card>
        <v-snackbar v-model="snackbar" :timeout="0" right vertical>
          {{ $t('addCatalog.formSuccess') }}
          <v-btn color="primary" text @click="$router.push('/db/' + url)">
            {{ $t('app.buttons.show') }}
          </v-btn>
